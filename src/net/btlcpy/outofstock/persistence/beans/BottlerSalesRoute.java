...
/**
 * A bean to represent Bottling Company bottler sales routes.
 * 
 * @author Ahmed Abd-Allah
 */
public class BottlerSalesRoute extends NamedUploadable
{
	// ----- Static members ---------------------------------------------------
	/** Database table name */
	public static final String tableName = "bttlrslsrts";
	...
	{
		if (cache == null)
			cache = new BeanCache(connection, tableName, null, BottlerSalesRoute.class, null, "name");
		return cache;
	}

	/** The number of active bottler sales routes (cached) */
	private static int totalActive = 0;
	public static int getTotalActive() { return totalActive; }
	/**
	 * Update the cached number of active bottler sales routes. Should be called when a new store list
